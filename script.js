document.addEventListener('DOMContentLoaded', () => {
    ...
    async function lerProximoVersiculo() {
        ...
        try {
            ...
            const onAudioErrorCallback = (errorEvent) => {
                ...
                if (estadoLeitura !== 'parado') { // Só para se não já estiver parado
    ...
    // *** FUNÇÃO DE ÁUDIO (CHAMA O BACKEND E USA DATA URL PARA CACHE - CORRIGIDA) ***
    function lerTexto(texto, onEndedCallback, onErrorCallback) { 

        // Limpa áudio anterior se existir E estiver a tocar/pausado
        if (audioAtual && (isAudioPlaying || estadoLeitura === 'pausado')) {
            console.warn("Áudio anterior existente, parando antes de iniciar novo.");
            if(audioAtual && !audioAtual.paused) {
        ...
        if (isProcessingAudio && abortController) {
             ...
             abortController.abort();
             isProcessingAudio = false; // Permite nova tentativa
        }
        ...
        if (!textoSanitizado) {
             ...
             isProcessingAudio = false;
             toggleControlButtons(false);
             if (onEndedCallback) { // Adiciona verificação
                try { onEndedCallback(); } catch(e) { console.error("Erro no onEndedCallback (texto vazio)", e); }
             }
        ...
        if (audioCache.has(cacheKey)) {
             ...
             return new Promise((resolve, reject) => {
                 ...
                 const handleErrorCache = (e) => {
                    ...
                    toggleControlButtons(false); 
                    if (onErrorCallback) { // Verifica se existe
                        try { onErrorCallback(e); } catch(err) { console.error("Erro no onErrorCallback (cache)", err); }
                    }
                    reject(e);
                 };
                 ...
                 const handleEndedCache = (e) => {
                    ...
                    if (onEndedCallback) { // Verifica se existe
                        try { onEndedCallback(e); } catch(err) { console.error("Erro no onEndedCallback (cache)", err); }
                    }
                    resolve();
                 };
                 ...
                 audioAtual.play().then(() => {
                     ...
                     handleErrorCache({ target: audioAtual, type: 'play_error', error: playError }); // Passa evento simulado
                 });
             });
        }
        ...
        .then(data => {
             // Verifica se ainda devemos tocar (pode ter sido interrompido durante o fetch)
             if (estadoLeitura !== 'tocando') {
                ...
                return new Promise((resolve, reject) => {
                    // *** CORREÇÃO TYPEERROR: Handlers definidos ANTES e mais robustos ***
                     const handleErrorBackend = (e) => {
                        console.error("Erro no elemento Audio (backend):", e ? e.type : 'Evento desconhecido');
                        isAudioPlaying = false; isProcessingAudio = false;
                        ...
                        if (onErrorCallback) { // Verifica se existe
                           try { onErrorCallback(e); } catch(err) { console.error("Erro no onErrorCallback (backend)", err); }
                        }
                        reject(e);
                    };
                    ...
                    const handleEndedBackend = (e) => {
                        ...
                        if (onEndedCallback) { // Verifica se existe
                           try { onEndedCallback(e); } catch(err) { console.error("Erro no onEndedCallback (backend)", err); }
                        }
                        resolve();
                    };
                    ...
                    audioAtual.play().then(() => {
                        ...
                        handleErrorBackend({ target: audioAtual, type: 'play_error', error: playError }); // Passa evento simulado
                    });
                });
        ...
        .catch(error => {
            // Se o erro foi 'Leitura interrompida...', não mostra alerta
            if (error && error.message === "Leitura interrompida antes de tocar.") {
                ...
                alert(`Não foi possível obter o áudio do servidor: ${error ? error.message : 'Erro desconhecido'}`);
                console.error(`Erro durante a chamada/processamento para índice ${indiceVersiculoAtual}:`, error);
            }

            // Garante liberação em caso de erro real (não AbortError)
